...
Buffer Buffer::createTempFileForSharedBuffer(bool writable) {
    ...
    int res = ftruncate(file, 2 * BufferSize);  // Use double-buffering: Allocate twice the BufferSize
    if (res != 0) {
        close(file);
    ...
    // Map it
    void *buffer = mmap(nullptr, 2 * BufferSize, writable ? (PROT_READ | PROT_WRITE) : PROT_READ, MAP_SHARED, file, 0);
...
Buffer Buffer::map_shared_buffer(std::string file_name, bool writable) {
    ...
    void *buffer = mmap(nullptr, 2 * BufferSize, writable ? PROT_WRITE : PROT_READ, MAP_SHARED, file, 0);
    if (buffer == MAP_FAILED) {
      close(file);
      throw std::runtime_error("Failed to memory-map file");
    }
