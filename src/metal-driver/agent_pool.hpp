...
class AgentPool {
 public:
  ...
  void reset();
  std::vector<std::shared_ptr<RegisteredAgent>> cachedPipeline() { return _pipeline_agents; }

 protected:
  void sendAllAgentsInvalid(std::unordered_set<std::shared_ptr<RegisteredAgent>> &agents);
