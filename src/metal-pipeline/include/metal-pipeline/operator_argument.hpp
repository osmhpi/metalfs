...
using OperatorArgumentValue =
    std::variant<uint32_t, bool, std::shared_ptr<std::vector<char>>>;

// Order matches the order of types above
enum class OptionType : size_t { Uint, Bool, Buffer };

class METAL_PIPELINE_API OperatorOptionDefinition {
 public:
  OperatorOptionDefinition()
      : OperatorOptionDefinition(0, static_cast<OptionType>(0), "", "", "") {}
  OperatorOptionDefinition(
      uint64_t offset, OptionType type, const std::string &key,
      const std::string &shrt, const std::string &description,
