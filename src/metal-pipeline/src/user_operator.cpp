...
UserOperator::UserOperator(std::string id, const std::string &manifest)
    : _manifest(jv_null()), _id(std::move(id)), _is_prepared(false) {
...
UserOperator::~UserOperator() { jv_free(_manifest); }
...
void UserOperator::configure(SnapAction &action) {
  ...
  for (const auto &option : _options) {
    ...
    metadata[3] =
        htobe32(prepare_required());  // enables preparation mode for the
    ...
    switch ((OptionType)option.second.value().index()) {
      case OptionType::Uint: {
        ...
        memcpy(job_config + configuration_data_offset, &value,
               sizeof(uint32_t));
      ...
      case OptionType::Buffer: {
        ...
        auto &buffer = *std::get<std::shared_ptr<std::vector<char>>>(
            option.second.value());
        memcpy(job_config + configuration_data_offset, buffer.data(),
               buffer.size());
...
void UserOperator::finalize(SnapAction &action) { (void)action; }
      ...
      if (type_str == "buffer") {
        ...
        _optionDefinitions.insert(std::make_pair(
            ...
            OperatorOptionDefinition(offset, OptionType::Buffer,
                                     jv_string_value(key), shortK, description,
                                     size)));
      }
    }
