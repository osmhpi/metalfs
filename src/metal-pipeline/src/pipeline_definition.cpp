...
PipelineDefinition::PipelineDefinition(
  ...
  if (_operators.size() < 2) {
    throw std::runtime_error(
        "Pipeline must contain at least data source and data sink operators.");
  }
...
uint64_t PipelineDefinition::run(SnapAction &action) {
  for (const auto &op : _operators) op->configure(action);
  ...
  spdlog::debug(
      ...
      "(Address: {:x}, Size: {}, Type: {})",
      _dataSource->address().addr, _dataSource->address().size,
      (uint64_t)_dataSource->address().type, _dataSink->address().addr,
      _dataSink->address().size, (uint64_t)_dataSink->address().type);
  action.execute_job(fpga::JobType::RunOperators, nullptr,
                     _dataSource->address(), _dataSink->address(), enable_mask,
                     /* perfmon_enable = */ 1, &output_size);

  for (const auto &op : _operators) op->finalize(action);
...
void PipelineDefinition::configureSwitch(SnapAction &action, bool set_cached) {
  ...
  auto *job_struct =
      reinterpret_cast<uint32_t *>(snap_malloc(sizeof(uint32_t) * 8));
  const uint32_t disable = 0x80000000;
  for (int i = 0; i < 8; ++i) job_struct[i] = htobe32(disable);

  uint8_t previous_op_stream =
      !_operators.empty() ? _operators[0]->internal_id() : 0;
  for (const auto &op : _operators) {
