...
static void snap_prepare_configure_streams_job(struct snap_job *cjob, metalfpga_job_t *mjob, int config)
{
    ...
    *job_struct_enable = htobe64(enable_mask);
    ...
    if (config == 0) {
        job_struct[0] = htobe32(0x80000000); // read_mem 0 -> passthrough 1
        job_struct[1] = htobe32(0); // passthrough 1 -> change_case 2
...
int main(int argc, char *argv[])
{
    ...
    // Pipeline test data
    uint8_t *in_data = (uint8_t*)snap_malloc(64);
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            in_data[i * 8 + j] = 'a' + i * 8 + j;
        }
    }
    ...
    uint8_t *out_data = (uint8_t*)snap_malloc(65);
    ...
    if (true) {
        ...
        // printf("Copying back to host ram...\n");
        snap_prepare_configure_streams_job(&cjob, &mjob, 1);
        rc = snap_action_sync_execute_job(action, &cjob, timeout);
        if (rc != 0) {
            fprintf(stderr, "err: job execution %d: %s!\n", rc,
                    strerror(errno));
