...
static bf_halfBlock_t bf_f(bf_halfBlock_t h, bf_SiC_t iCpy, bf_key_t &key)
{
...
// #pragma HLS UNROLL factor=16 LW: Can not unroll because of bf_f() dependencies
        left ^= key.g_P[i];
        right ^= bf_f(left, iCpy, key);
        right ^= key.g_P[i+1];
        left ^= bf_f(right, iCpy, key);
    }
    left ^= key.g_P[16];
    right ^= key.g_P[17];
...
mtl_retc_t op_blowfish_encrypt_set_key(snap_membus_t &line) {
    snapu8_t keyBytes = 16;
    snapu8_t keyHBlockCount = keyBytes >> BF_HBLOCK_BADR_BITS;
    bf_halfBlock_t key[BF_BPL];
    bf_splitKeyLine(line, key, keyHBlockCount);
    bf_keyInit(key, encrypt_key);
...
void op_blowfish_encrypt(mtl_stream &in, mtl_stream &out, snap_bool_t enable) {
    if (enable) {
        for (;;) {
            ...
            bf_encrypt(left, right, 0, encrypt_key);
...
void op_blowfish_decrypt(mtl_stream &in, mtl_stream &out, snap_bool_t enable) {
    if (enable) {
        for (;;) {
            ...
            bf_decrypt(left, right, 0, decrypt_key);
