...
void op_mem_read_impl(snap_membus_t *din_gmem, hls::stream<word_stream_element> &mem_stream, mtl_mem_configuration &config, snap_bool_t enable) {
    if (enable) {
...
#pragma HLS dataflow
    ...
    op_mem_read_impl(din_gmem, mem_stream, config, enable);
