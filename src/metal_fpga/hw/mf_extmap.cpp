                    ...
                    snap_membus_t * mem)
{
    ...
    map.current_pblock = map.extent_begin[0];
}
...
#pragma HLS UNROLL factor=16
        uint64_t extent_begin = mf_extmap_firstlblock[i_extent];
...
mf_bool_t mf_extmap_next(mf_extmap_t & map) {
    ...
    snapu64_t boffset = map.current_lblock - mf_extmap_firstlblock(map.current_extent);
...
snapu64_t mf_extmap_remaining_blocks(mf_extmap_t & map, snapu64_t lblock) {
    ...
    snapu64_t btoblk = lblock - current_lblock;
