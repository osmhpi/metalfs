#include "hls_operator_blowfish_decrypt.h"
...
static bf_key_t decrypt_key;

static bf_halfBlock_t bfd_bf_f(bf_halfBlock_t h, bf_SiC_t iCpy, bf_key_t &key)
{
    bf_SiE_t a = (bf_SiE_t)(h >> 24),
             b = (bf_SiE_t)(h >> 16),
             c = (bf_SiE_t)(h >> 8),
             d = (bf_SiE_t) h;
    return ((key.g_S[iCpy][0][a] + key.g_S[iCpy][1][b]) ^ key.g_S[iCpy][2][c]) + key.g_S[iCpy][3][d];
}

static void bfd_bf_encrypt(bf_halfBlock_t & left, bf_halfBlock_t & right, bf_SiC_t iCpy, bf_key_t &key)
{
    // printf("bfd_bf_encrypt(0x%08x, 0x%08x, %d)", *((uint32_t *)(void *)&left), *((uint32_t *)(void *)&right), *((uint32_t *)(void *)iCpy));

BF_ENCRYPT:
    for (int i = 0; i < 16; i += 2) {
// #pragma HLS UNROLL factor=16 LW: Can not unroll because of bfd_bf_f() dependencies
        left ^= key.g_P[i];
        right ^= bfd_bf_f(left, iCpy, key);
        right ^= key.g_P[i+1];
        left ^= bfd_bf_f(right, iCpy, key);
    }
    left ^= key.g_P[16];
    right ^= key.g_P[17];
...
static void bfd_bf_decrypt(bf_halfBlock_t & left, bf_halfBlock_t & right, bf_SiC_t iCpy, bf_key_t &key)
{
...
static void bfd_bf_keyInit(bf_halfBlock_t key[16], bf_key_t &dest)
{
    // printf("bfd_bf_keyInit() <- \n");

    // init P and S from initP, initS and key
    for (int i = 0; i < 16; ++i) {
        dest.g_P[i] = c_initP[i] ^ key[i];
    }
    for (int n = 0; n < 4; ++n) {
        for (int i = 0; i < 256; ++i) {
...
#pragma HLS UNROLL factor=8 //==BF_S_CPYCNT
                dest.g_S[iCpy][n][i] = c_initS[n][i];
            }
        }
    }

    // chain decrypt 0-Block to replace P and S entries
    bf_halfBlock_t left = 0, right = 0;
    for (int i = 0; i < 16; i += 2) {
        bfd_bf_encrypt(left, right, 0, dest);
        dest.g_P[i] = left;
        dest.g_P[i+1] = right;
    }
    for (int n = 0; n < 4; ++n) {
        for (int i = 0; i < 256; i += 2) {
            ...
            {
#pragma HLS UNROLL factor=8 //BF_S_CPYCNT
                dest.g_S[iCpy][n][i] = left;
                dest.g_S[iCpy][n][i+1] = right;
            }
        }
    }
}
...
void hls_operator_blowfish_decrypt(mtl_stream &in, mtl_stream &out, snapu32_t prepare, snapu32_t key_bytes[16]) {
    ...
    if (prepare) {
        bfd_bf_keyInit(key_bytes, decrypt_key);
    } else {
        mtl_stream_element element;
        do {
            element = in.read();

            bf_halfBlock_t left = element.data(63, 32);
            bf_halfBlock_t right = element.data(31, 0);

            bfd_bf_decrypt(left, right, 0, decrypt_key);

            element.data(63, 32) = left;
            element.data(31, 0) = right;
