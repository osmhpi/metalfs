...
static int apply_config(struct snap_action *action) {
    ...
    if (_length && _offset && false) { // As long as we don't support byte-enabled writes to DRAM, this doesn't make sense
        ...
        rc = 0; // snap_action_sync_execute_job(action, &cjob, timeout);
