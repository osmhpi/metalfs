...
static mtl_retc_t action_configure_streams(snapu32_t *switch_ctrl, snap_membus_512_t * mem_in, const uint64_t job_address) {
    // Everything fits into one memory line
    snap_membus_512_t line = mem_in[MFB_ADDRESS(job_address)];
...
static mtl_retc_t action_map(snap_membus_512_t * mem_in, const uint64_t job_address)
{
    snap_membus_512_t line = mem_in[MFB_ADDRESS(job_address)];
...
static void configure_operators(snapu32_t *operator_ctrl, snap_membus_512_t * mem_in, const uint64_t job_address) {
    snap_membus_512_t line = mem_in[MFB_ADDRESS(job_address)];
    ...
    for (auto i = 0; i < length; ++i, ++offset, membus_line_offset += sizeof(snapu32_t)) {
        #pragma HLS pipeline
        if (membus_line_offset % sizeof(snap_membus_512_t) == 0) {
            line = mem_in[MFB_ADDRESS(job_address) + membus_line_offset / sizeof(snap_membus_512_t)];
        }
...
mtl_retc_t process_action(snap_membus_512_t * mem_in,
                          ...
                          action_reg * act_reg)
{
    ...
    case JobType::ReadImageInfo:
    {
        ...
        snap_membus_512_t current_line = 0;
        int i = 0;
        for (; i < read_len; i += sizeof(uint32_t)) {
            #pragma HLS PIPELINE
            int lineoffset = i % sizeof(snap_membus_512_t);
            current_line(31 + 8*lineoffset, 8*lineoffset) = image_info_ctrl[(4096 + i) / sizeof(uint32_t)];

            if (lineoffset == sizeof(snap_membus_512_t) - sizeof(uint32_t)) {
                mem_in[MFB_ADDRESS(act_reg->Data.job_address) + (i / sizeof(snap_membus_512_t))] = current_line;
                current_line = 0;
            }
        }

        if (i % sizeof(snap_membus_512_t)) {
            mem_in[MFB_ADDRESS(act_reg->Data.job_address) + (i / sizeof(snap_membus_512_t))] = current_line;
        }
    ...
    {
        snap_membus_512_t line = mem_in[MFB_ADDRESS(act_reg->Data.job_address)];
